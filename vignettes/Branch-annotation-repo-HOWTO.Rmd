...
As of release 3.21 Annotations are around 89G. See also inst/script/tempS3 for
...
These instructions use BioC 3.22 as the new devel version and nebbiolo2 as the
...
```sh
rsync -ave ssh . biocpush@nebbiolo2:PACKAGES/3.22/data/annotation/
```
...
```sh
rsync --dry-run -ave ssh biocpush@nebbiolo1:PACKAGES/3.21/data/annotation/ .
```
...
```sh
/extra/www/bioc/packages/3.22/data/annotation -> ../../3.21/data/annotation
```
...
1.  Remove any deprecated packages from the 3.21 annotation repo that should not
    be included in the devel 3.22 repo.
...
\~/cron.log/3.22/propagate-data-annotation-\* log file.
...
## Update bioconductor.org/config.yaml to build landing pages for 3.22
