...
vignette: >
  ...
  %\VignetteIndexEntry{The Release Checklist}
...
After stopping release builds, on all Mac build machines, point the "Current"
symlink in /Library/Frameworks/R.framework/Versions to the new DEVEL version of
R. Otherwise we will be producing broken binaries! (Not applicable if only one
build is running on each Mac machine, as should be the case from BioC 2.9
onward; but we do want to make sure that there is only ever one version of R
installed on these Mac build machines.)
...
* Branch creation must be done after all packages for 3.6 have been added to the
manifest.
...
built off their master branch so do **NOT** touch the `BBS_BIOC_GIT_BRANCH`
variable!
...
- Bump versions and create BioC 3.6 release branch
  <https://github.com/Bioconductor/ReleaseLaunch/blob/master/vignettes/Version-bump-and-branch-creation-HOWTO.Rmd>
