...
For example, for the BioC 3.18 release, we will need to do this for all
the packages listed in the `software.txt`, `data-experiment.txt`,
`workflows.txt`, and `books.txt` files of the `RELEASE_3_18` branch
...
before the BioC 3.18 builds start for software, data-experiment, workflow, and
book packages.

Look for the prerun jobs in the crontab for the `biocbuild` user on the central
BioC 3.18 builder to get the times the software and data-experiment builds get
...
No 3.18 build should start until:
...
everything is fine. Otherwise, comment out all the builds that are at the risk
of starting before step F3 is completed.
...
The `RELEASE_3_17` lines in `gitolite-admin/conf/packages.conf` were commented
...
NOTE: Do not change the branch from RELEASE_3_17 to RELEASE_3_18, it is
not a good solution. Maintainers now will be able to push their own
RELEASE_3_18 branch before we are able to create it at release
...
### C5. Login to the machine where you've performed the preliminary steps
...
- Test that a non-super user can push access is enabled on the dummy package
  BiocGenerics_test.
...
DON'T FORGET THIS STEP! Its purpose is to make the BioC 3.18 builds grab the
`RELEASE_3_18` branch of all packages instead of their `devel` branch.

Login to the central BioC 3.18 builder as `biocbuild` and replace
...
select 'Contents' \> 'Read and Write'. If you are also modifying default
branches, select 'Read and Write' under 'Administration'. It is recommended to
use `gitcreds::gitcreds_set()` to store the fine-grained PAT.
