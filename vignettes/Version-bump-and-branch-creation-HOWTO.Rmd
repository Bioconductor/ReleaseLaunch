...
For example, for the BioC 3.23 release, we will need to do this for
all the packages listed in the `software.txt`, `data-annotation.txt`,
`data-experiment.txt`, `workflows.txt`, and `books.txt` files of
the `RELEASE_3_23` branch of the `manifest` repo. However, note that
...
release, and BEFORE THE BioC 3.23 BUILDS START FOR SOFTARE,
...
the central BioC 3.23 builder to get the times the software and
...
* Update this document to reflect the BioC version to be released i.e.
  replace all occurrences of `3.23` and `RELEASE_3_23` with appropriate
...
* Populate `git.bioconductor.org` with git clones of the `manifest` repo
  ...
  # clone `manifest` repo
  $BBS_HOME/utils/update_bioc_git_repos.py manifest RELEASE_3_23
  
  # clone software package repos (takes approx. 2.5h)
  time $BBS_HOME/utils/update_bioc_git_repos.py software devel RELEASE_3_23
  
  # clone data-annotation package repos (takes a couple of minutes)
  time $BBS_HOME/utils/update_bioc_git_repos.py data-annotation devel RELEASE_3_23
  
  # clone data-experiment package repos (takes approx. 2.5h)
  time $BBS_HOME/utils/update_bioc_git_repos.py data-experiment devel RELEASE_3_23
  
  # clone workflow package repos (takes approx. 9 min)
  time $BBS_HOME/utils/update_bioc_git_repos.py workflows devel RELEASE_3_23
  
  # clone book repos (takes < 1 min)
  time $BBS_HOME/utils/update_bioc_git_repos.py books devel RELEASE_3_23
  ```
...
### C1. Comment out all builds in crontab on central BioC 3.23 builder

**IMPORTANT: No 3.23 build should start until:**

- the bump and branch process is complete,

- **and** `BBS_BIOC_GIT_BRANCH` is switched from `devel` to `RELEASE_3_23`
  on the central BioC 3.23 builder (this switch is documented in section
  F3 below).
...
The `RELEASE_3_22` lines in `gitolite-admin/conf/packages.conf` were
...
NOTE: Do not change the branch from RELEASE_3_22 to RELEASE_3_23, it
is not a good solution. Maintainers now will be able to push their
own RELEASE_3_23 branch before we are able to create it at release
...
Or (trying as of release 3.23) try using hooks exclusion file. See
...
### C6. Checkout/update the `RELEASE_3_23` branch of the `manifest` repo

```sh
cd ~/git.bioconductor.org/manifest
git pull --all
git checkout RELEASE_3_23
git branch
git status
```
...
- `MANIFEST_FILE`: Point `MANIFEST_FILE` to the manifest file for software
  packages. This must be the file from the `RELEASE_3_23` branch of
  the `manifest` repo:
...
$BBS_HOME/utils/bump_version_and_create_branch.py --push RELEASE_3_23 $pkgs_in_manifest >bump_version_and_create_branch.log 2>&1 &
```

Alternatively, the `bump_version_and_create_branch.py` run above can be
replaced with a 2-pass run:

```sh
# First pass (apply all the changes but do NOT push them):
$BBS_HOME/utils/bump_version_and_create_branch.py RELEASE_3_23 $pkgs_in_manifest >bump_version_and_create_branch.log1 2>&1 &
# Second pass (push all the changes):
$BBS_HOME/utils/bump_version_and_create_branch.py --push RELEASE_3_23 $pkgs_in_manifest >bump_version_and_create_branch.log2 2>&1 &
```
...
* In the 2-pass run, the second pass checks the packages and applies the
  ...
  `RELEASE_3_23` branch) before pushing the changes.
...
$BBS_HOME/utils/bump_version_and_create_branch.py --no-bump --push RELEASE_3_23 $pkgs_in_manifest >bump_version_and_create_branch.log 2>&1 &
```
...
### F1. Enable push access to new `RELEASE_3_23` branch
...
- If not done already, replace all instances of `RELEASE_3_22` with
  `RELEASE_3_23`.

- Uncomment all `RELEASE_3_23` and `devel` lines.
...
### F3. Switch `BBS_BIOC_GIT_BRANCH` from `devel` to `RELEASE_3_23` on central BioC 3.23 builder

DON'T FORGET THIS STEP! Its purpose is to make the BioC 3.23 builds grab
the `RELEASE_3_23` branch of all packages instead of their `devel` branch.

Log on the central BioC 3.23 builder as `biocbuild` and replace
...
in `~/BBS/3.23/config.bat`

Then remove the `manifest` and `MEAT0` folders from `~/bbs-3.23-bioc/`,
`~/bbs-3.23-data-annotation/`, `~/bbs-3.23-data-experiment/`,
`~/bbs-3.23-workflows/`, and `~/bbs-3.23-books/`. They'll get automatically
...
resource owner, then 'All repositories' under 'Repository access'. Finally,
click on the `Generate token` green button. Use `gitcreds::gitcreds_set()` to
store the fine-grained PAT.

---

**NOTES**

* If you are renaming branches, select 'Read and Write' under
'Contents' in the 'Repository permissions' section. 

* Depending on the organization's settings, R / Bioconductor
...
```r
add_gh_release_branches(release = "RELEASE_3_23", org = "Bioconductor")
```
...
add_gh_release_branch(
    package_name = "updateObject", release = "RELEASE_3_23", gh_branch = "devel"
)
```
